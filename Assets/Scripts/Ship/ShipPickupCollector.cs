            ...
            if (otherCollider.TryGetComponent(out HealthPickup pickup))
            {
                switch(pickup.Type)
                {
                    case PickupType.Health:
                        shipController.Repair(pickup.Value, DamageType.Hull);
                        shipController.UpdateHealthBar(shipController.Configuration.Health, shipController.Configuration.MaxHealth);
                        break;
                    default:
