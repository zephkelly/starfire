...
namespace StarfireV2
{
    ...
    public class InvestigateGoal : Goal
    {
        ...
        public override bool IsComplete(BTContext context)
        {
            ...
            if (!context.TryGet<V2DetectedEntity>("investigation_target", out var target))
            {
                // No target - investigation might be complete or need re-evaluation
                return false;
            }

            var goalLevel = InvestigateParams?.goalV2DetectionLevel ?? V2DetectionLevel.Silhouette;
