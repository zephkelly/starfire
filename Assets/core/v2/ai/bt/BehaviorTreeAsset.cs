    ...
    public class BehaviorTreeAsset : ScriptableObject
    {
        ...
        public IBTNode CreateRuntimeTree()
        {
            ...
            foreach (var data in nodes)
            {
                ...
                {
                    failedCount++;
                    Debug.LogWarning($"[BehaviorTreeAsset] '{name}': Failed to create node id='{data.id}', type={data.nodeType}, actionType='{data.actionType}'");
                }
            }
            Debug.Log($"[BehaviorTreeAsset] '{name}': Created {nodeMap.Count}/{nodes.Count} nodes ({failedCount} failed)");
