...
namespace StarfireV2
{
    ...
    public class SelectClosestContactAction : BTAction
    {
        ...
        {
            if (!Context.TryGet<List<V2DetectedEntity>>(_inputKey, out var contacts) || contacts.Count == 0)
            {
#if UNITY_EDITOR
...
#if UNITY_EDITOR
            if (Context.DebugLogging)
                Debug.Log($"[BT:{Context.EntityName}] SelectClosestContact: SUCCESS - Selected {closest.Transform?.name} at {closest.Distance:F1}m, level {closest.Level}");
#endif
